...
export class DbTestLoaderMulti {
  ...
  constructor(
    ...
    // Log configuration
    console.log(
      `Source database: ${this.sourceDb} on ${this.sourceParsed.hostname}`
    )
    console.log(
      `Destination database: ${this.destDb} on ${this.destParsed.hostname}`
    )
  ...
  initializeLoaders(sourceFixture?: string, destFixture?: string): boolean {
    ...
    try {
      this.sourceLoader = new DbTestLoader(
        this.sourceUrl,
        this.prismaSchemaPath,
        sourceFixture
      )

      this.destLoader = new DbTestLoader(
        this.destUrl,
        this.prismaSchemaPath,
        destFixture
      )
  ...
  async checkDependencies(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `✓ Source PostgreSQL connection: ${version.substring(0, 50)}...`
      )
    ...
    if (this.destAdminUrl !== this.sourceAdminUrl) {
      try {
        ...
        const version = result.rows[0].version
        console.log(
          `✓ Destination PostgreSQL connection: ${version.substring(0, 50)}...`
        )
    ...
    try {
      ...
      console.error(
        "✗ Prisma CLI not found: Install with: npm install -g prisma"
      )
      return false
    }
  ...
  async cleanupTestDatabases(): Promise<boolean> {
    ...
    try {
      ...
      if (successSource && successDest) {
        ...
        console.warn(
          "⚠ Some test databases may not have been cleaned up properly"
        )
        return false
      }
    } catch (error) {
