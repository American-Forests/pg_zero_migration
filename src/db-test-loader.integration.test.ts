...
describe("DbTestLoader Integration Tests", () => {
  ...
  const testDbName = `test_db_loader_${Date.now()}_${Math.random()
    .toString(36)
    .substr(2, 9)}`
  ...
  describe("Error Handling Integration", () => {
    it("should handle invalid database connection gracefully", async () => {
      ...
      await expect(invalidLoader.connect()).rejects.toThrow(
        DatabaseConnectionError
      )
    }, 30000)
    ...
    it("should handle missing schema file", async () => {
      ...
      await expect(
        loaderWithInvalidSchema.setupDatabaseSchema()
      ).rejects.toThrow(SchemaSetupError)
    ...
    it("should handle missing fixture file", async () => {
      ...
      await expect(loaderWithInvalidFixture.loadTestData()).rejects.toThrow(
        DataLoadingError
      )
  ...
  describe("TES Schema PostGIS Integration", () => {
    const tesTestDbName = `test_tes_db_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 9)}`
    ...
    it("should handle TES schema with PostGIS geometry types", async () => {
      ...
      try {
        ...
        throw new Error(
          `PostGIS spatial functions not working: ${spatialError}`
        )
      }
