...
describe("DbTestLoaderMulti", () => {
  ...
  describe("Constructor - URL Parsing and Database Naming", () => {
    it("should parse URLs correctly for same host scenario", () => {
      const dbMulti = new DbTestLoaderMulti(
        testSourceUrl,
        testSourceUrl,
        testSchemaPath
      )
      ...
      expect(dbMulti.destUrl).toBe(
        "postgresql://user:pass@localhost:5432/testdb_migration_dest"
      )
    ...
    it("should parse URLs correctly for different host scenario", () => {
      ...
      expect(dbMulti.destUrl).toBe(
        "postgresql://user:pass@remote:5432/destdb_migration_dest"
      )
      expect(dbMulti.sourceParsed.hostname).toBe("localhost")
    ...
    it("should throw MultiDatabaseError for invalid URLs", () => {
      ...
      expect(
        () => new DbTestLoaderMulti(invalidUrl, undefined, testSchemaPath)
      ).toThrow(MultiDatabaseError)
    })
    ...
    it("should build admin URLs correctly", () => {
      ...
      expect(dbMulti.sourceAdminUrl).toBe(
        "postgresql://user:pass@localhost:5432/postgres"
      )
      expect(dbMulti.destAdminUrl).toBe(
        "postgresql://user:pass@remote:5432/postgres"
      )
    })
    ...
    it("should handle URLs with different ports", () => {
      ...
      expect(dbMulti.destUrl).toBe(
        "postgresql://user:pass@remote:5434/destdb_migration_dest"
      )
    })
  })
  ...
  describe("Database Operations", () => {
    ...
    it("should handle database creation errors", async () => {
      ...
      const dbMultiTest = new DbTestLoaderMulti(
        testSourceUrl,
        testDestUrl,
        testSchemaPath
      )
