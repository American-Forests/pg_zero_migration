...
export class DbSchemaParser {
  ...
  static parse(schemaPath: string): DatabaseSchema {
    ...
    for (const match of modelMatches) {
      ...
      try {
        ...
        throw new DbSchemaParseError(
          `Failed to parse model ${modelName}`,
          error as Error
        )
      }
    }
  ...
  private static parseModel(
    ...
    for (const line of fieldLines) {
      ...
      const { column, foreignKey, index } = this.parseField(
        fieldName,
        fieldDefinition,
        enumTypes
      )
  ...
  private static parseField(
    ...
    const column: ColumnDefinition = {
      ...
      autoIncrement: !!(
        ...
        (attributes.id && baseType === "Int")
      ), // @id Int fields or explicit @default(autoincrement())
   ...
   */
  private static parseFieldAttributes(
    fieldDefinition: string
  ): PrismaFieldAttribute {
    ...
    // @default(value)
    const defaultMatch = fieldDefinition.match(
      /@default\s*\(\s*((?:[^()]+|\([^()]*\))+)\s*\)/
    )
    if (defaultMatch) {
