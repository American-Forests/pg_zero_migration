...
import { DatabaseSchema } from '../util/db-schema-types.js';
...
export class DbTestFixtureLoader {
  ...
  async loadFixtureFile(fixturePath: string): Promise<void> {
    ...
    try {
      ...
      throw new FixtureLoadError(`Failed to load fixture file: ${fixturePath}`, error as Error);
    }
  }
   ...
   */
  private async loadTableData(tableName: string, records: Record<string, any>[]): Promise<void> {
  ...
  private async insertRecord(
    ...
    if (Object.keys(filteredRecord).length === 0) {
      console.warn(`No valid columns found for record in table ${tableName}:`, record);
      return;
    }
    ...
    try {
      ...
      throw new FixtureLoadError(
        `Failed to insert record into ${tableName}: ${JSON.stringify(filteredRecord)}`,
        error as Error
      );
    }
  }
  ...
  private async updateSequences(fixtureData: FixtureData): Promise<void> {
    for (const tableName of Object.keys(fixtureData)) {
      ...
      const autoIncrementColumns = table.columns.filter(col => col.autoIncrement);
   ...
   */
  async verifyDataCounts(expectedCounts: Record<string, number>): Promise<boolean> {
