...
describe('Migration CLI Integration Tests', () => {
  ...
  const testPgPassword = process.env.TEST_PGPASSWORD || '';
  ...
  beforeEach(async () => {
    ...
    multiLoader = new DbTestLoaderMulti('tes', expectedSourceUrl, expectedDestUrl);
  ...
  it('should create migration log file when running CLI migration command', async () => {
    ...
    const args = ['tsx', migrationScript, 'migrate', expectedSourceUrl, expectedDestUrl];
    ...
    expect(logContent).toMatch(/completed successfully \(\d+ms\)/);
